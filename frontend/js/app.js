...
document.getElementById('register-form').addEventListener('submit', async (e) => {
  ...
  try {
    const data = await api('/auth/register', { method: 'POST', body: { first_name, last_name, username, password } });
    onAuthSuccess(data);
  } catch (err) {
...
function showMainView(view, profileUsername) {
  ...
  document.getElementById('explorer-panel').classList.toggle('hidden', view !== 'explorer');
  document.getElementById('profile-panel').classList.toggle('hidden', view !== 'profil');
  document.getElementById('admin-panel').classList.toggle('hidden', view !== 'admin');
  ...
  setBottomNavActive(view === 'messages' ? 'messages' : view === 'profil' || view === 'admin' ? 'profil' : view === 'explorer' ? 'explorer' : 'accueil');
}
...
const BADGE_SEAL_PATH = 'M12.0,1.5 Q12.0,1.5 13.43,2.33 Q14.87,3.16 16.52,3.34 Q18.17,3.51 18.85,5.02 Q19.52,6.53 20.76,7.65 Q21.99,8.76 21.65,10.38 Q21.3,12.0 21.65,13.62 Q21.99,15.24 20.76,16.36 Q19.52,17.47 18.85,18.98 Q18.17,20.49 16.52,20.67 Q14.87,20.84 13.43,21.67 Q12.0,22.5 10.57,21.67 Q9.13,20.84 7.48,20.67 Q5.83,20.49 5.16,18.98 Q4.48,17.47 3.25,16.36 Q2.01,15.24 2.36,13.62 Q2.7,12.0 2.36,10.38 Q2.01,8.76 3.25,7.65 Q4.48,6.53 5.16,5.02 Q5.83,3.51 7.48,3.34 Q9.13,3.16 10.57,2.33 Z';

function badgeSvg() {
  return `<svg viewBox="0 0 24 24" width="14" height="14" style="display:inline-block; vertical-align:-2px;">
    <path d="${BADGE_SEAL_PATH}" fill="currentColor"/>
    <path d="M8 12.2l2.6 2.6L16.2 9" stroke="var(--bg)" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"/>
  </svg>`;
}

function badgeHtml(badge) {
  if (badge === 'gold') return `<span class="badge badge-gold" title="Admin">${badgeSvg()}</span>`;
  if (badge === 'diamond') return `<span class="badge badge-diamond" title="VIP">${badgeSvg()}</span>`;
  if (badge === 'blue') return `<span class="badge badge-check" title="Vérifié">${badgeSvg()}</span>`;
  return '';
}
...
async function loadAdminPanel() {
  ...
  panel.innerHTML = `
    <div class="admin-stats">
      <div class="admin-stat-card"><b>${stats.users}</b><span>Utilisateurs</span></div>
      <div class="admin-stat-card"><b>${stats.posts}</b><span>Publications</span></div>
      <div class="admin-stat-card"><b>${stats.messages}</b><span>Messages</span></div>
...
function renderAdminUserRow(u) {
  ...
  row.innerHTML = `
    ...
    <select class="admin-badge-select" ${isSelf ? 'disabled' : ''}>
      ...
      <option value="gold" ${u.badge === 'gold' ? 'selected' : ''}>👑 Admin</option>
      <option value="diamond" ${u.badge === 'diamond' ? 'selected' : ''}>💎 VIP</option>
      <option value="blue" ${u.badge === 'blue' ? 'selected' : ''}>✔️ Vérifié</option>
    </select>
    <button class="admin-block-btn ${u.is_blocked ? 'is-blocked' : ''}" ${isSelf ? 'disabled' : ''}>
      ${u.is_blocked ? 'Débloquer' : 'Bloquer'}
    </button>
  `;

  row.querySelector('.admin-badge-select').addEventListener('change', async (e) => {
    await api(`/admin/users/${u.id}/badge`, { method: 'PATCH', body: { badge: e.target.value || null } });
    showToast(`Badge mis à jour pour ${u.username}`);
  });

  row.querySelector('.admin-block-btn').addEventListener('click', async (e) => {
    const newState = !u.is_blocked;
    await api(`/admin/users/${u.id}/block`, { method: 'PATCH', body: { blocked: newState } });
    u.is_blocked = newState;
    e.target.classList.toggle('is-blocked', newState);
    e.target.textContent = newState ? 'Débloquer' : 'Bloquer';
    showToast(newState ? `${u.username} a été bloqué` : `${u.username} a été débloqué`);
  });
...
async function loadExplorerPanel() {
  ...
  document.getElementById('explorer-search-input').addEventListener('input', async (e) => {
    ...
    if (!q) {
      ...
      discoverBox.classList.remove('hidden');
      return;
    }
    ...
    searchBox.innerHTML = '';
    if (users.length === 0) {
      searchBox.innerHTML = '<div class="feed-empty">Aucun utilisateur trouvé.</div>';
      return;
    }
  ...
  if (posts.length === 0) {
    discoverBox.innerHTML = '<div class="feed-empty">Aucune publication à découvrir pour le moment.</div>';
  } else {
    posts.forEach((p) => discoverBox.appendChild(renderPostCard(p)));
  }
}

function renderExplorerUserRow(u) {
  const row = document.createElement('div');
  ...
  `;
  row.addEventListener('click', () => {
    showMainView('profil', u.username);
  });
  return row;
}
...
function renderProfileHeader(p) {
  ...
  if (p.relationship === 'me') {
    ...
    if (me.is_admin) actionsHtml += `<button class="profile-edit-btn" id="open-admin-btn">🛠️ Panel Admin</button>`;
  ...
  card.innerHTML = `
    ...
    <div class="profile-name">${p.username} ${badge}</div>
    ${p.first_name ? `<div style="color:var(--text-dim); font-size:12.5px; margin-top:-2px;">${escapeHtml(p.first_name)} ${escapeHtml(p.last_name || '')}</div>` : ''}
    ${p.bio ? `<div class="profile-bio">${escapeHtml(p.bio)}</div>` : ''}
    <div class="profile-stats">
      <div class="profile-stat"><b>${p.post_count}</b><span>Posts</span></div>
      <div class="profile-stat" id="stat-followers" style="cursor:pointer;"><b>${p.follower_count}</b><span>Abonnés</span></div>
      <div class="profile-stat" id="stat-following" style="cursor:pointer;"><b>${p.following_count}</b><span>Abonnements</span></div>
    </div>
    <div class="profile-actions">${actionsHtml}</div>
  `;

  card.querySelector('#stat-followers').addEventListener('click', () => openFollowListModal(p.username, 'followers'));
  card.querySelector('#stat-following').addEventListener('click', () => openFollowListModal(p.username, 'following'));

  const editBtn = card.querySelector('#edit-profile-btn');
  if (editBtn) editBtn.addEventListener('click', () => openEditProfileModal(p));

  const adminBtn = card.querySelector('#open-admin-btn');
  if (adminBtn) adminBtn.addEventListener('click', () => showMainView('admin'));
...
async function openFollowListModal(username, kind) {
  ...
  title.textContent = kind === 'followers' ? 'Abonnés' : 'Abonnements';
  body.innerHTML = '<div class="feed-empty">Chargement...</div>';
  modal.classList.remove('hidden');

  const list = await api(`/users/${encodeURIComponent(username)}/${kind}`);
  body.innerHTML = '';

  if (list.length === 0) {
    body.innerHTML = `<div class="feed-empty">${kind === 'followers' ? 'Aucun abonné pour le moment.' : 'Aucun abonnement pour le moment.'}</div>`;
    return;
  }
...
document.getElementById('follow-list-modal').addEventListener('click', (e) => {
  if (e.target.id === 'follow-list-modal') e.target.classList.add('hidden');
});
